...
solve inp = unlines [
  ...
  where
    ...
    step ch coord = step' [maxY,0,maxY] initBlizzs [coord]
      where
        initBlizzs = M.map (:[]) . M.filter (`elem` "<>^v") $ ch

        step' [] blz coords = []
        step' (goalY:goalYs) blz coords
          | length reachedGoal > 0 = step' goalYs blz reachedGoal
          where
            reachedGoal = filter ((==goalY) . snd) coords
        step' goals blz coords = blz':step' goals blz' coords'
          where
